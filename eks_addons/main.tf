...
module "argo_rollouts" {
  ...
  chart_version    = try(var.argo_rollouts.chart_version, "2.41.0")
...
module "argo_workflows" {
  ...
  chart_version    = try(var.argo_workflows.chart_version, "1.0.18")
...
module "argocd" {
  ...
  chart_version    = try(var.argocd.chart_version, "9.7.1") # TODO - v6.x
...
module "argo_events" {
  ...
  chart_version    = try(var.argo_events.chart_version, "2.4.22")
...
locals {
  aws_efs_csi_driver_controller_service_account = try(
    var.aws_efs_csi_driver.controller_service_account_name,
    "efs-csi-controller-sa"
  )

  aws_efs_csi_driver_node_service_account = try(
    var.aws_efs_csi_driver.node_service_account_name,
    "efs-csi-node-sa"
  )

  aws_efs_csi_driver_namespace = try(
    var.aws_efs_csi_driver.namespace,
    "kube-system"
  )

  efs_arns = try(
    var.aws_efs_csi_driver.efs_arns,
    [
      "arn:${local.partition}:elasticfilesystem:${local.region}:${local.account_id}:file-system/*"
    ]
  )

  efs_access_point_arns = try(
    var.aws_efs_csi_driver.efs_access_point_arns,
    [
...
data "aws_iam_policy_document" "aws_efs_csi_driver" {
  ...
  source_policy_documents = try(
    var.aws_efs_csi_driver.source_policy_documents,
    []
  )

  override_policy_documents = try(
    var.aws_efs_csi_driver.override_policy_documents,
    []
  )
...
module "aws_efs_csi_driver" {
  ...
  # Disable Helm release when Kubernetes resources are not being created.
  create_release = var.create_kubernetes_resources
  ...
  description = try(
    ...
    "A Helm chart to deploy aws-efs-csi-driver"
  )

  namespace = local.aws_efs_csi_driver_namespace

  create_namespace = try(
  ...
  role_permissions_boundary_arn = try(
    var.aws_efs_csi_driver.role_permissions_boundary_arn,
    null
  )

  role_description = try(
    var.aws_efs_csi_driver.role_description,
    "IRSA for aws-efs-csi-driver project"
  )

  # Map of additional managed policy names to policy ARNs.
  role_policies = try(
    var.aws_efs_csi_driver.role_policies,
    {}
  )

  # The generated EFS policy is passed as a source policy document.
  source_policy_documents = data.aws_iam_policy_document.aws_efs_csi_driver[*].json

  # Must be a list, not an object/map.
  policy_statements = try(
    var.aws_efs_csi_driver.policy_statements,
    []
  )
  ...
  policy_description = try(
    var.aws_efs_csi_driver.policy_description,
    "IAM Policy for AWS EFS CSI Driver"
  )
...
module "aws_load_balancer_controller" {
  ...
  chart_version    = try(var.aws_load_balancer_controller.chart_version, "3.4.0")
...
resource "aws_autoscaling_lifecycle_hook" "aws_node_termination_handler" {
  for_each = {
  for k, v in var.aws_node_termination_handler_asg_arns :
  k => v if var.enable_aws_node_termination_handler
}
...
module "aws_node_termination_handler" {
  ...
  chart_version    = try(var.aws_node_termination_handler.chart_version, "0.27.6")
...
module "aws_privateca_issuer" {
  ...
  chart_version    = try(var.aws_privateca_issuer.chart_version, "v1.9.0")
...
module "external_dns" {
  ...
  chart_version    = try(var.external_dns.chart_version, "1.21.1")
...
module "external_secrets" {
  ...
  chart_version    = try(var.external_secrets.chart_version, "2.6.0")
...
module "gatekeeper" {
  ...
  chart_version    = try(var.gatekeeper.chart_version, "3.22.2")
...
module "ingress_nginx" {
  ...
  chart_version    = try(var.ingress_nginx.chart_version, "4.15.1")
...
module "karpenter" {
  ...
  chart_version    = try(var.karpenter.chart_version, "1.13.0")
...
module "metrics_server" {
  ...
  chart_version    = try(var.metrics_server.chart_version, "3.13.1")
...
module "secrets_store_csi_driver" {
  ...
  chart_version    = try(var.secrets_store_csi_driver.chart_version, "1.6.0")
...
module "secrets_store_csi_driver_provider_aws" {
  ...
  chart_version    = try(var.secrets_store_csi_driver_provider_aws.chart_version, "3.1.1")
...
locals {
  velero_name            = "velero"
  velero_service_account = try(
    var.velero.service_account_name,
    "${local.velero_name}-server"
  )

  velero_backup_s3_bucket = try(
    split(":", var.velero.s3_backup_location),
    []
  )

  velero_backup_s3_bucket_arn = try(
    split("/", var.velero.s3_backup_location)[0],
    var.velero.s3_backup_location,
    ""
  )

  velero_backup_s3_bucket_name = try(
    split("/", local.velero_backup_s3_bucket[5])[0],
    local.velero_backup_s3_bucket[5],
    ""
  )

  velero_backup_s3_bucket_prefix = try(
    split("/", var.velero.s3_backup_location)[1],
    ""
  )
...
resource "aws_iam_policy" "velero" {
  ...
  name_prefix = "${local.cluster_name}-velero-"
...
################################################################################
# Velero Helm Addon and IRSA Role
################################################################################
...
module "velero" {
  ...
  description      = try(var.velero.description, "A Helm chart to install Velero")
  namespace        = local.velero_namespace
  create_namespace = try(var.velero.create_namespace, true)
  chart            = try(var.velero.chart, "velero")
  chart_version    = try(var.velero.chart_version, "12.1.0")
  ...
  # Velero chart 12.x expects backupStorageLocation and
  # volumeSnapshotLocation to be arrays.
  values = concat(
    [
      yamlencode({
        configuration = {
          backupStorageLocation = [
            {
              name     = "default"
              provider = "aws"
              bucket   = local.velero_backup_s3_bucket_name
              prefix   = local.velero_backup_s3_bucket_prefix
              default  = true
  ...
  timeout                    = try(var.velero.timeout, 900)
  ...
  # Do not merge values from a previously failed Velero release.
  reuse_values = try(var.velero.reuse_values, false)
  reset_values = try(var.velero.reset_values, true)
  ...
  # Do not add backupStorageLocation or volumeSnapshotLocation here.
  # Chart 12.x requires those values to be supplied as arrays through values.
  set = concat(
    [
      {
        name = "initContainers"
  ...
  role_permissions_boundary_arn = try(var.velero.role_permissions_boundary_arn, null)
...
################################################################################
# Attach Velero IAM Policy
################################################################################

resource "aws_iam_role_policy_attachment" "velero" {
  count = var.enable_velero ? 1 : 0

  role       = module.velero.iam_role_name
...
module "vpa" {
  ...
  chart_version    = try(var.vpa.chart_version, "4.12.2") # TODO - 4.0
...
module "aws_gateway_api_controller" {
  ...
  chart_version    = try(var.aws_gateway_api_controller.chart_version, "v2.1.1") # TODO - 1.0
